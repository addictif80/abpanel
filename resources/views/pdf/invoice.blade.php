...
<div class="parties">
    <div class="party">
        ...
        <div class="party-detail">
            {{ $settings['company_legal_form'] ?? 'Micro-entreprise' }}<br>
            SIREN : {{ $settings['company_siren'] }}
            @if(!empty($settings['company_rcs']))<br>{{ $settings['company_rcs'] }}@endif
        </div>
        @endif
        @if(!empty($settings['company_iban']))
        <div class="party-detail" style="margin-top:6px;">
            IBAN : {{ $settings['company_iban'] }}<br>
            @if(!empty($settings['company_bic']))BIC : {{ $settings['company_bic'] }}@endif
        </div>
        @endif
    </div>
    <div class="party">
        <div class="label">Facturé à</div>
        <div class="party-name">{{ $invoice->user->full_name }}</div>
        <div class="party-detail">
            {{ $invoice->user->email }}
