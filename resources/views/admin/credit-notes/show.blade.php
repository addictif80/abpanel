...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    ...
    <div class="space-y-5">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-2">
            ...
            <div class="bg-green-50 border border-green-100 rounded-lg p-3 text-xs text-green-700 mb-2">
                Appliquer cet avoir déduira <strong>{{ number_format($creditNote->amount, 2) }} {{ $creditNote->currency }}</strong>
                du solde de la facture <strong>{{ $creditNote->invoice->number }}</strong>
                (solde actuel : {{ number_format($creditNote->invoice->total, 2) }} {{ $creditNote->invoice->currency }}).
                @if($creditNote->amount >= $creditNote->invoice->total)
                La facture sera automatiquement soldée.
                @endif
            </div>
            <form method="POST" action="{{ route('admin.credit-notes.apply', $creditNote) }}"
                onsubmit="return confirm('Appliquer cet avoir sur la facture {{ $creditNote->invoice->number }} ?')">
                @csrf
                <button class="w-full px-3 py-2 text-sm text-green-700 bg-green-50 border border-green-100 rounded-lg hover:bg-green-100 transition text-left">
                    Appliquer l'avoir sur la facture
                </button>
            </form>
            @endif
        </div>
