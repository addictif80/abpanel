        ...
        <div x-show="tab === 'mail'" x-data="{ ...testConnection('mail'), testEmail: '{{ auth()->user()->email }}' }">
            <form method="POST" action="{{ route('admin.settings.mail') }}" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Rétention des journaux mails (mois)</label>
                        <input type="number" name="mail_logs_retention_months" min="1" max="120" value="{{ $settings['mail_logs_retention_months'] ?? 12 }}"
                            class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                        <p class="text-xs text-gray-400 mt-1">RGPD : les journaux mails (contenu complet des emails envoyés) plus anciens sont supprimés automatiquement chaque nuit.</p>
                    </div>
                </div>
