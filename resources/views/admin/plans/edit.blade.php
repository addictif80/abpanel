...
<form method="POST" action="{{ route('admin.plans.update', $plan) }}" class="max-w-2xl space-y-5"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="type" required x-model="planType"
                    ...
                    <option value="service" {{ old('type', $plan->type) === 'service' ? 'selected' : '' }}>Service (cloud, mail, gestionnaire de mots de passe, ...)</option>
                </select>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4" x-data="{ selectedGroup: '{{ old('ldap_group', $plan->ldap_group) }}' }">
        ...
        <div x-data="{ requiredGroup: '{{ old('requires_ldap_group', $plan->requires_ldap_group) }}' }">
            ...
            <p class="text-xs text-gray-400 mt-1">Prérequis actuel : <strong>{{ $plan->requires_ldap_group ?: '(aucun)' }}</strong>. Si renseigné, le client doit déjà avoir une facture payée pour un plan de ce groupe avant de pouvoir commander celui-ci.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input type="number" name="limit_per_domain" value="{{ old('limit_per_domain', $plan->limit_per_domain) }}"
            ...
            <div>
                ...
                <input type="number" name="limit_per_subdomain" value="{{ old('limit_per_subdomain', $plan->limit_per_subdomain) }}"
                    min="1" placeholder="Non limité"
                    class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
