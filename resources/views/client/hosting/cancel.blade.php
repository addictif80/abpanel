@extends('layouts.app')
@section('title', 'Résilier ' . $hosting->domain)
...
<div class="max-w-lg mx-auto">
    ...
    <div class="mt-4 bg-red-50 border border-red-200 rounded-xl p-5 mb-6">
        <div class="flex items-start gap-3">
            ...
            <div>
                <h2 class="font-bold text-red-800 text-base">Résiliation définitive — {{ $hosting->domain }}</h2>
                <p class="text-sm text-red-700 mt-1">
                    Cette action est <strong>irréversible</strong>. Votre hébergement et <strong>toutes les données associées</strong>
                    seront supprimés définitivement de CyberPanel.
                </p>
                <ul class="mt-3 space-y-1 text-sm text-red-700 list-disc list-inside">
                    <li>Tous vos fichiers web et bases de données seront supprimés</li>
                    <li>Vos adresses email associées au domaine seront perdues</li>
                    <li>L'hébergement ne pourra pas être restauré</li>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <form method="POST" action="{{ route('client.hosting.cancelConfirm', $hosting) }}">
            @csrf
            <div class="mb-5">
            ...
            <button type="submit"
                onclick="return confirm('Dernière confirmation : supprimer définitivement l\'hébergement « {{ addslashes($hosting->domain) }} » et toutes ses données ?')"
        ...
        <div class="mt-4 text-center">
            <a href="{{ route('client.hosting.cancel', $hosting) }}" class="text-sm text-gray-500 hover:text-gray-700 underline">
                Renvoyer un nouveau code
            </a>
            <span class="text-gray-300 mx-2">·</span>
            <a href="{{ route('client.hosting.index') }}" class="text-sm text-gray-500 hover:text-gray-700 underline">
                Annuler
            </a>
        </div>
    </div>
